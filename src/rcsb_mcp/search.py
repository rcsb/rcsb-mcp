...
# The pre-composer tool names, kept DISPATCHABLE but UNLISTED.
#
# These are the seven flat rcsb_search_* tools the rcsb_query_* layer replaced. A client
# that fetched its tool list before the rename still holds those names and will keep
# calling them until it refreshes; deleting them turns that into a hard failure for a
# caller that did nothing wrong. Hidden from tools/list, they cost nothing on the token
# surface — a description that is never sent is never paid for — while still answering.
#
# They are not thin wrappers around the new path but the original implementations, which
# now route through queries.build_search_request like everything else, so the bodies they
# produce are byte-identical to the pre-refactor ones (proved for all 37 cases in
# tests/test_query_baseline.py). Nothing about them needs to be kept in step by hand.
#
# Retire them once client tool-list caches have plausibly turned over.
_LEGACY_SEARCH_TOOLS = (
...
def register_search_tools(mcp) -> None:
    """Attach the RCSB Search API tools to a FastMCP server.

    The rcsb_query_* / rcsb_search_request layer is advertised; the superseded
    rcsb_search_* names are registered but hidden, so a client holding a stale tool list
    keeps working. Hiding needs RcsbFastMCP (see rcsb_mcp.tooling); on a plain FastMCP —
    which the unit tests build — the legacy names are simply not registered, so a test
    server advertises exactly what a fresh client sees.
    """
    for fn in _SEARCH_TOOLS:
        mcp.tool(annotations=READ_ONLY)(fn)
    if not hasattr(mcp, "hide_tool"):
        return
    for fn in _LEGACY_SEARCH_TOOLS:
        mcp.tool(annotations=READ_ONLY)(fn)
        mcp.hide_tool(fn.__name__)
